...
export default class LoggingSession {
  ...
  startTrackingDocumentChanges() {
    ...
    _loggingSession.changeHandlerDisposable = _loggingSession._textBuffer.onDidChange(function(event) {
      const ts = Date.now();
  ...
  startSession(originalSourceFileString = '') {
    ...
    _this.textHandlerDisposable = atom.workspace.observeActiveTextEditor(() => {this.updateTextEditor();}, atom.workspace.getActiveTextEditor());
  }
  updateTextEditor(){
