...
setMarker(marker){
  ...
  updateEditor(){
    ...
    const marker = activeEditor.markScreenRange(activeEditor.cursors[0].getCurrentWordBufferRange());
...
getColumn(actualPoint){
 ...
 return Math.floor((actualPoint.x - this.offsetX) / this.columnWidth);
  ...
  getLineColumn(windowPoint) {
    ...
    var column = this.getColumn(actualPoint)
    const mousePosition = this.editor.component.screenPositionForMouseEvent({clientX: windowPoint.x, clientY: windowPoint.y});
    const bufferPosition = this.editor.bufferPositionForScreenPosition(mousePosition, {clipDirection: "closest"});
